...
const getError = (fn: () => void): Error | void => {
  try {
    fn();
  } catch (err) {
    return err;
...
export default class Test {
  ...
  throws(actual: () => void, message: string = 'should throw') {
    const err = getError(actual);
    this.custom(!!err, { operator: 'throws', message, actual: err });
  }

  notThrows(actual: () => void, message: string = 'should throw') {
    const err = getError(actual);
