...
class Test {
  ...
  custom (assert: boolean, { operator, actual, expected, message }: CustomMetaType) {
    if (assert) {
      ...
      this._result.fail(message, { operator, actual, expected });
    }
  }
  ...
  truthy(actual: any, message: string = 'should be truthy') {
    this.custom(
      !!actual,
      { operator: 'truthy', message, expected: String(true), actual: objectInspect(actual) }
    );
  }
  ...
  falsy(actual: any, message: string = 'should be falsy') {
    this.custom(
      !actual,
      { operator: 'falsy', message, expected: String(false), actual: objectInspect(actual) }
    );
  }
  ...
  true(actual: any, message: string = 'should be true') {
    this.custom(
      actual === true,
      { operator: 'true', message, expected: String(true), actual: objectInspect(actual) }
    );
  }
  ...
  false(actual: any, message: string = 'should be false') {
    this.custom(
      actual === false,
      { operator: 'false', message, expected: String(false), actual: objectInspect(actual) }
    );
  }
}
